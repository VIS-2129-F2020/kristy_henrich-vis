...
ggplot(nhoods) +
  ...
  geom_sf(data = lib, color = "yellow", size = 0.01) +
  theme_map() +
  annotation_scale()
```
...
```{r}
left_side  <- st_bbox(polls)$xmin
top_side <- st_bbox(polls)$ymax
...
## Figure 5. Average number of A points in each C polygon
Average number of public libraries in each neighborhood

```{r}
nhoods <- nhoods %>%
  mutate(num_pub_lib = lengths(st_covers(nhoods, lib)))
```

```{r}
ggplot(nhoods) +
  geom_sf(color = NA, 
          aes(fill = num_pub_lib)) +
